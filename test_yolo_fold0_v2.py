...
def process_dicom_for_yolo(series_path: str, mode: str = "2.5D") -> List[np.ndarray]:
    ...
    if mode == "2D":
        ...
        rgb_slices = []
        for i in range(1, len(all_frames)-1):  # Match validation script: range(1, len-1)
            try:
                # Get 3 consecutive slices: [i-1, i, i+1], handling boundaries (match validation script)
                slice_indices_3 = []
                for offset in [-1, 0, 1]:
                    idx = i + offset
                    if 0 <= idx < len(all_frames):
                        ...
                        # For boundary cases, use the center slice (match validation script)
                ...
                prev_frame = all_frames[slice_indices_3[0]]
                curr_frame = all_frames[slice_indices_3[1]]
                next_frame = all_frames[slice_indices_3[2]]
