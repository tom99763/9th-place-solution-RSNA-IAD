...
class ClassificationValidator(BaseValidator):
    ...
    def get_desc(self):
        ...
        return ("%22s" + "%11s" * 6) % ("classes", "top1_acc", "top5_acc", "precision", "recall", "f1", "fitness")
    ...
    def init_metrics(self, model):
        ...
        self.metrics.names = model.names  # Set names for compatibility with callbacks
        self.metrics.nc = self.nc  # Set number of classes
        self.pred = []
        self.targets = []
    ...
    def print_results(self):
        ...
        pf = "%22s" + "%11.3g" * 6  # print format for enhanced metrics
        LOGGER.info(pf % ("all", self.metrics.top1, self.metrics.top5, 
                         self.metrics.mean_precision, self.metrics.mean_recall, 
                         self.metrics.mean_f1, self.metrics.fitness))
        
        # Print per-class results if verbose and multiple classes
        if self.args.verbose and not self.training and self.nc > 1 and len(self.metrics.precision) > 0:
            LOGGER.info("Per-class metrics (precision, recall, f1):")
            pf_cls = "%22s" + "%11.3g" * 3
            for i in range(self.nc):
                if i < len(self.metrics.precision):  # Safety check
                    class_name = self.names.get(i, f"class_{i}")
                    LOGGER.info(pf_cls % (f"{class_name} ({i})",
                                          self.metrics.precision[i],
                                          self.metrics.recall[i],
                                          self.metrics.f1[i]))
