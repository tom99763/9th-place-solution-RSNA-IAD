...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN, ConvNeXtBlock
from .conv import Conv, DWConv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init

__all__ = "Detect", "DetectConvNeXt", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment"
...
class Detect(nn.Module):
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            ...
            b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (960 / s) ** 2)  # cls (.01 objects, 80 classes, 960 img)
        if self.end2end:
...
class DetectConvNeXt(nn.Module):
    """YOLO Detect head using ConvNeXt blocks for enhanced feature processing."""
    ...
    def __init__(self, nc=80, ch=()):
        """Initialize the YOLO detection layer with ConvNeXt blocks."""
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(ch)  # number of detection layers
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        ...
        #self.cv2 = nn.ModuleList(
        #    nn.Sequential(
        #        nn.Conv2d(x, c2, 1),  # Channel transition: x -> c2
        #        ConvNeXtBlock(c2),     # Operates on c2
        #        ConvNeXtBlock(c2),
        #        nn.Conv2d(c2, 4 * self.reg_max, 1)  # Output
        #    ) for x in ch
        #)
        #
        ## Classification head
        ...
        # Box regression head - no bottleneck, direct channel operation
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                ConvNeXtBlock(x),
                ConvNeXtBlock(x),   
        ...
        # Classification head - no bottleneck, direct channel operation  
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                ConvNeXtBlock(x),  # Operate directly on input channels
                ConvNeXtBlock(x),
                nn.Conv2d(x, self.nc, 1)  # Final projection
            ) for x in ch
        )
...
class WorldDetect(Detect):
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            a[-1].bias.data[:] = 1.0  # box
            # b[-1].bias.data[:] = math.log(5 / m.nc / (960 / s) ** 2)  # cls (.01 objects, 80 classes, 960 img)
...
class YOLOEDetect(Detect):
    ...
    def bias_init(self):
        ...
        for a, b, c, s in zip(m.cv2, m.cv3, m.cv4, m.stride):  # from
            ...
            # b[-1].bias.data[:] = math.log(5 / m.nc / (960 / s) ** 2)  # cls (.01 objects, 80 classes, 960 img)
            b[-1].bias.data[:] = 0.0
            c.bias.data[:] = math.log(5 / m.nc / (960 / s) ** 2)
