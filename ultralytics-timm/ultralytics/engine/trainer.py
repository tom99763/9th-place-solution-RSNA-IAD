...
class BaseTrainer:
    ...
    def _do_train(self, world_size=1):
        ...
        while True:
            ...
            self._model_train()
            if RANK != -1:
                print(f"[TRAINER DEBUG] Calling set_epoch({epoch}) on DDP sampler")
                self.train_loader.sampler.set_epoch(epoch)
            elif hasattr(self.train_loader.sampler, 'set_epoch'):
                print(f"[TRAINER DEBUG] Calling set_epoch({epoch}, rank={RANK}) on custom sampler")
                self.train_loader.sampler.set_epoch(epoch, rank=RANK)
    ...
    def build_optimizer(self, model, name="auto", lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        if name == "auto":
            ...
            name, lr, momentum = ("AdamW", lr_fit, 0.9)#("SGD", 0.01, 0.9) if iterations > 10000 else ("AdamW", lr_fit, 0.9)
