...
class FocalLossWithLabelSmoothing(nn.Module):
    """
    Focal Loss combined with Label Smoothing for classification.
    ...
    def forward(self, inputs, targets):
        ...
        # Apply alpha weighting (optional, mainly for binary classification)
        if self.alpha != 1.0:
...
#    """Criterion class for computing training losses for classification with Focal Loss and Label Smoothing."""
#    
#    def __init__(self, alpha=1.0, gamma=2.0, smoothing=0.1):
#        """
#        Args:
#            alpha (float): Alpha parameter for focal loss
#           gamma (float): Gamma parameter for focal loss (higher = more focus on hard examples)
...
#        loss = self.criterion(preds, batch["cls"])
