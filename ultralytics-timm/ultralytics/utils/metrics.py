...
def compute_roc_auc(y_true, scores, eps=1e-16):
    """
    Compute ROC AUC given binary labels and corresponding scores.

    Args:
        y_true (np.ndarray): Binary ground-truth labels of shape (N,).
...
def ap_per_class(
    ...
    Returns:
        ...
        auc (np.ndarray): ROC AUC per class computed using IoU@0.5 correctness vs confidence.
    ...
    for ci, c in enumerate(unique_classes):
        ...
        # ROC AUC using IoU@0.5 correctness vs confidence
        try:
            auc[ci] = compute_roc_auc(tp[i][:, 0].astype(np.float64), conf[i].astype(np.float64))
        except Exception:
            auc[ci] = float("nan")
    ...
    return tp, fp, p, r, f1, ap, auc, unique_classes.astype(int), p_curve, r_curve, f1_curve, x, prec_values
...
class Metric(SimpleClass):
    ...
    def __init__(self) -> None:
        ...
        self.f1 = []  # (nc, )
        self.all_ap = []  # (nc, 10)
        self.auc = []  # (nc, ) ROC AUC at IoU@0.5 per class
    ...
    def mauc(self):
        """
        Return the mean ROC AUC (using IoU@0.5 correctness vs confidence) across classes.

        Returns:
            (float): Mean AUC across classes.
        """
        if len(self.auc):
            auc_arr = np.array(self.auc, dtype=np.float64)
            valid = np.isfinite(auc_arr)
            return float(auc_arr[valid].mean()) if valid.any() else 0.0
        return 0.0

    def mean_results(self):
        """Return mean of results, mp, mr, map50, map."""
        return [self.mp, self.mr, self.map50, self.map, self.mauc]

    def class_result(self, i):
        """Return class-aware result, p[i], r[i], ap50[i], ap[i], auc[i]."""
        return self.p[i], self.r[i], self.ap50[i], self.ap[i], self.auc[i]
    ...
    def fitness(self):
        ...
        w = [0.0, 0.0, 0.5, 0.25, 0.25]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95, mAUC]
...
class DetMetrics(SimpleClass):
    ...
    def keys(self):
        ...
        return ["metrics/precision(B)", "metrics/recall(B)", "metrics/mAP50(B)", "metrics/mAP50-95(B)", "metrics/mauc(B)"]

    def mean_results(self):
        """Calculate mean metrics & return precision, recall, mAP50, mAP50-95, and mAUC."""
...
class ClassifyMetrics(SimpleClass):
    """
    Class for computing comprehensive classification metrics.

    Attributes:
        top1 (float): The top-1 accuracy.
        top5 (float): The top-5 accuracy.
        precision (np.ndarray): Precision for each class.
        recall (np.ndarray): Recall for each class.
        ...
        mean_f1 (float): Mean F1-score across all classes.
        speed (dict): A dictionary containing the time taken for each step in the pipeline.
        task (str): The task type, set to 'classify'.
        nc (int): Number of classes.
        names (dict): Class names mapping for compatibility with callbacks.
    """

    def __init__(self, nc=0) -> None:
        ...
        self.task = "classify"
        self.nc = nc
        self.names = {}  # Class names mapping for compatibility with callbacks

    def process(self, targets, pred, conf=None):
        """
        Process target classes and predicted classes to compute comprehensive metrics.

        Args:
            targets (torch.Tensor): Target classes.
            pred (torch.Tensor): Predicted classes (logits or probabilities).
            conf (torch.Tensor, optional): Confidence scores for predictions.
        """
        ...
        if pred.dim() == 2 and pred.shape[1] > 1:
            # Check if predictions are class indices (integers) or probabilities/logits (floats)
            if pred.dtype in (torch.int32, torch.int64, torch.long):
                # Predictions are class indices (from classification validator)
                pred_classes = pred[:, 0]  # Take the top-1 prediction
                
                # For class indices, we can't compute confidence from the data alone
                ...
                # Compute per-class metrics using class indices
                self._compute_per_class_metrics_from_indices(targets, pred_classes)
                
            else:
                # Predictions are logits or probabilities (floats)
                pred_probs = torch.softmax(pred, dim=1) if pred.max() > 1.0 else pred
                pred_classes = torch.argmax(pred_probs, dim=1)
                
                # Confidence metrics removed
                
                # Compute top-k accuracies
                correct = (targets[:, None] == pred_classes[:, None]).float()
                acc = torch.stack((correct[:, 0], correct.max(1).values), dim=1)
                self.top1, self.top5 = acc.mean(0).tolist()
                
                # Compute per-class metrics
                self._compute_per_class_metrics(targets, pred_classes, pred_probs)
        else:
            # Legacy behavior for single predictions
            correct = (targets[:, None] == pred).float()
            acc = torch.stack((correct[:, 0], correct.max(1).values), dim=1)
            self.top1, self.top5 = acc.mean(0).tolist()

    def _compute_per_class_metrics(self, targets, pred_classes, pred_probs):
        """Compute per-class precision, recall, F1-score and confusion matrix."""
        targets_np = targets.cpu().numpy()
        pred_classes_np = pred_classes.cpu().numpy()
        
        # Get number of classes
        if self.nc == 0:
            self.nc = max(int(targets.max().item()), int(pred_classes.max().item())) + 1
        
        # Compute confusion matrix
        self.confusion_matrix = np.zeros((self.nc, self.nc), dtype=np.int32)
        for i in range(len(targets_np)):
            self.confusion_matrix[targets_np[i], pred_classes_np[i]] += 1
        
        # Compute per-class metrics
        self.precision = np.zeros(self.nc)
        self.recall = np.zeros(self.nc)
        self.f1 = np.zeros(self.nc)
        ...
        for i in range(self.nc):
            ...
            if self.precision[i] + self.recall[i] > 0:
                self.f1[i] = 2 * (self.precision[i] * self.recall[i]) / (self.precision[i] + self.recall[i])
    ...
    def _compute_per_class_metrics_from_indices(self, targets, pred_classes):
        """Compute per-class precision, recall, F1-score and confusion matrix from class indices."""
        targets_np = targets.cpu().numpy()
        pred_classes_np = pred_classes.cpu().numpy()
    ...
    @property
    def fitness(self):
        """Return weighted combination of metrics as fitness score."""
        # Weighted combination: top1 (40%), mean_f1 (30%), mean_precision (15%), mean_recall (15%)
        return self.mean_f1 # 0.5 * self.mean_f1 + 0.25 * self.mean_precision + 0.25 * self.mean_recall

    @property
    def results_dict(self):
        """Return a dictionary with model's performance metrics and fitness score."""
        metrics = [
            self.top1, self.top5, self.mean_precision, self.mean_recall, self.mean_f1,
            self.fitness
        ]
        return dict(zip(self.keys + ["fitness"], metrics))
    ...
    def keys(self):
        ...
        return [
            ...
            "metrics/precision_mean", "metrics/recall_mean", "metrics/f1_mean"
        ]
