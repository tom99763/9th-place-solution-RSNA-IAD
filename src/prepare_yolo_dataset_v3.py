...
def read_dicom_frames_hu_with_ds(path: Path) -> Tuple[List[np.ndarray], pydicom.Dataset]:
    """Read DICOM and return (frames_in_HU, dataset)."""
    ds = pydicom.dcmread(str(path), force=True)
    pix = ds.pixel_array
    slope = float(getattr(ds, "RescaleSlope", 1.0))
    intercept = float(getattr(ds, "RescaleIntercept", 0.0))
        ...
        if labels_for_series:
            ...
            for (sop, x, y, cls_id) in labels_for_series:
                ...
                if not dcm_path.exists():
                    continue
                frames, ds = read_dicom_frames_hu_with_ds(dcm_path)
                if not frames:
                    continue
                center, width = get_windowing_params(ds, frames[0])
                img = apply_windowing_or_normalize(frames[0], center, width)
