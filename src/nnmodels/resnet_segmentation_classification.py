# %load ../src/nnmodels/resnet_segmentation_classification.py
...
class SegmentationClassifier(nn.Module):
    ...
    """
    def __init__(self, model_name='resnet18d', pretrained=True):
        super().__init__()
        ...
        self.classification_head = nn.Linear(sum(encoder_channels), 14)

        # # The input to the linear layer is the number of channels in the last feature map
        # self.loc_head = nn.Linear(sum(encoder_channels), 13)
    ...
    def forward(self, x):
        ...
        x = x.expand(-1, 3, -1, -1)
        features = encode_for_resnet(self.encoder, x, B, depth_scaling=[2,2,2,2,1])
        
        pooled_feats = []
        for i in range(len(features)):
            features[i] = features[i].amax(dim=2)

            pooled_feats.append(self.flatten(self.avgpool(features[i])))


        global_feats = torch.cat(pooled_feats,dim=1)

        classification_output = self.classification_head(global_feats)
        ...
        return {
            "classification_logits": classification_output[:,0:1],
            "segmentation_logits": segmentation_output,
            "location_logits": classification_output[:,1:]
        }
