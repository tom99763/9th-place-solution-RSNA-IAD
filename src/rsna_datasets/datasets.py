...
class NpzVolumeDepthSliceDataset(Dataset):
    """
...
class NpzVolumeSliceMIPDataset(Dataset):
    """
    Dataset to load .npz image volumes and serve random 2D slices.
    """
    def __init__(self, uids, cfg, transform=None, mode="train"):
...
class NpzDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        self.train_dataset = NpzVolumeDepthSliceDataset(uids=list(train_uids), cfg=self.cfg,transform=self.train_transforms)
        self.val_dataset = NpzVolumeDepthSliceDataset(uids=list(val_uids), cfg=self.cfg, transform=self.val_transforms, mode="val")
