...
def resize_to_shape(vol: np.ndarray, target_shape: Tuple[int, int, int] = (64, 384, 384)) -> np.ndarray:
    """Resize volume to specified shape (D, H, W) using scipy.ndimage."""
    Z, H, W = vol.shape
    target_d, target_h, target_w = target_shape
    if (Z, H, W) == (target_d, target_h, target_w):
        return vol
    zoom_factors = (target_d / Z, target_h / H, target_w / W)
    return ndimage.zoom(vol, zoom_factors, order=1, prefilter=False).astype(np.float32)
...
def plot_planes_grid(
    ...
    overlay_texts: Dict[str, List[List[str]]] | None = None,
):
        ...
        if i < len(imgs):
            ...
            if overlays:
                ...
                if i < len(pts_list) and pts_list[i]:
                    ...
                    # Optionally annotate points with provided texts
                    if overlay_texts:
                        txt_list = overlay_texts.get(key, [])
                        if i < len(txt_list) and txt_list[i]:
                            for (x, y), t in zip(pts_list[i], txt_list[i]):
                                ax.text(
                                    x + 2,
                                    y - 2,
                                    t,
...
def main():
    ...
    print(f"Resampled isotropic shape: {vol_iso.shape}")

    # Resize to 64x384x384
    vol_iso = resize_to_shape(vol_iso, target_shape=(64, 384, 384))
    print(f"Final resized shape: {vol_iso.shape}")
    ...
    if lbl_path.exists():
        try:
            ...
            for _, r in df.iterrows():
                ...
                # map to cube indices (already using final cube dims W,H,Z)
                xi, yi, zi = x0 * kx, y0 * ky, z0 * kz
                print("resized (x,y,z):", xi, yi, zi)
                ...
                if zi_r in z_to_pos:
                    ...
                    overlay_texts["axial"][z_to_pos[zi_r]].append(f"({xi:.1f}, {yi:.1f}, {zi:.1f})")
                if yi_r in y_to_pos:
                    overlays["coronal"][y_to_pos[yi_r]].append((xi, zi_flip))
                    overlay_texts["coronal"][y_to_pos[yi_r]].append(f"({xi:.1f}, {yi:.1f}, {zi:.1f})")
                if xi_r in x_to_pos:
                    overlays["sagittal"][x_to_pos[xi_r]].append((yi, zi_flip))
                    overlay_texts["sagittal"][x_to_pos[xi_r]].append(f"({xi:.1f}, {yi:.1f}, {zi:.1f})")
...
# python3 -m src.vizualize_views\ copy --series-dir /home/sersasj/RSNA-IAD-Codebase/data/series/1.2.826.0.1.3680043.8.498.98671147049544538232951626931886481868 --slice-mode quartile --no-show --save outputs/series_quartiles_512cube.png