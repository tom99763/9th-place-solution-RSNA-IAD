...
def create_auc_evolution_plot(auc_tracking: Dict, save_path: str = '', show_plot: bool = False, plane: str = ''):
    ...
    loc_macro_aucs = auc_tracking['loc_macro_aucs']
    
    # Plot both AUC curves
    plt.plot(series_counts, cls_aucs, 'b-o', label='Classification AUC', linewidth=2, markersize=6)
    plt.plot(series_counts, loc_macro_aucs, 'r-s', label='Location Macro AUC', linewidth=2, markersize=6)
    ...
    plt.title(f'AUC Evolution During Validation - {plane.capitalize()} Plane' if plane else 'AUC Evolution During Validation', fontsize=14)
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
    
    # Set y-axis limits to focus on relevant range
    all_aucs = [auc for auc in cls_aucs + loc_macro_aucs if not math.isnan(auc)]
    if all_aucs:
    ...
    if cls_aucs and not math.isnan(cls_aucs[-1]):
        plt.annotate(f'Final Cls AUC: {cls_aucs[-1]:.3f}', 
                    xy=(series_counts[-1], cls_aucs[-1]), 
                    xytext=(10, 10), textcoords='offset points',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='lightblue', alpha=0.7),
                    fontsize=10)
    
    if loc_macro_aucs and not math.isnan(loc_macro_aucs[-1]):
        plt.annotate(f'Final Loc AUC: {loc_macro_aucs[-1]:.3f}', 
                    xy=(series_counts[-1], loc_macro_aucs[-1]), 
...
def main():
    ...
    for sid in tqdm(val_series, desc=f"Validating {plane}", unit="series"):
        ...
        # Periodic partial AUC (cls and loc macro) tracking
        if len(cls_labels) % args.plot_interval == 0 and len(cls_labels) >= args.plot_interval:
            ...
            auc_tracking['loc_macro_aucs'].append(loc_macro_pauc)
    ...
    if len(cls_labels) > 0:
        ...
        if not auc_tracking['series_counts'] or auc_tracking['series_counts'][-1] != final_count:
            ...
            auc_tracking['loc_macro_aucs'].append(loc_macro_auc)
    
    # Create AUC evolution plot
    if (args.save_plot or args.show_plot) and len(auc_tracking['series_counts']) > 1:
        create_auc_evolution_plot(auc_tracking, args.save_plot, args.show_plot, plane)
