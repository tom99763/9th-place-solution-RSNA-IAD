...
def validation(cfg: DictConfig) -> None:
    ...
    pred_cls_probs = {"CTA": [], "MR": []}
    pred_loc_probs = {"CTA": [], "MR": []}
    ...
    for uid in tqdm(val_uids):
        ...
        with np.load(f"./data/processed/slices/{uid}.npz") as data:
            ...
            key = "MR" if "MR" in modality else "CTA"

            cls_labels[key].append(rowdf["Aneurysm Present"].iloc[0])
            loc_labels[key].append(loc_label)
            pred_cls_probs[key].append(cls_prob)
            pred_loc_probs[key].append(loc_probs)



    pred_cls_probs["CTA"] = rankdata(pred_cls_probs["CTA"])
    pred_cls_probs["CTA"] /= len(pred_cls_probs["CTA"])

    pred_cls_probs["MR"] = rankdata(pred_cls_probs["MR"])
    pred_cls_probs["MR"] /= len(pred_cls_probs["MR"])

    cls_labels = np.concatenate([cls_labels["CTA"], cls_labels["MR"]])
    pred_cls_probs = np.concatenate([pred_cls_probs["CTA"], pred_cls_probs["MR"]])
    
    loc_labels = np.concatenate([np.stack(loc_labels["CTA"]),np.stack(loc_labels["MR"])], axis=0)
    pred_loc_probs = np.concatenate([np.stack(pred_loc_probs["CTA"]), np.stack(pred_loc_probs["MR"])], axis=0)
